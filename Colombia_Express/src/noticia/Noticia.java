...
public class Noticia extends Metodos implements FieldChangeListener
{
	...
	public void cargarDatos(){
		...
		try{     		
            ...
            while(sc.next()){
				...
				allContent.setMargin(0, lateral, 0, lateral);	
				...
				//Status.show(imgCut.substring(cut-3, cut) + " holach ",1000);
				...
				//if(!limite.equals("png")){
					...
					//Status.show( rc.getString(5) ,500);
					//bitmapImage = Bitmap.getBitmapResource("rounded-border1.png");
				//}else{
					//bitmapImage = ImageUrl("http://www.i-moves.com/demos/promosApp/IMG/1363215936_Oferta.jpg");
					//bitmapImage = Bitmap.getBitmapResource("rounded-border1.png");
				//Dialog.alert(vectorImage.elementAt(k).toString());
					bitmapImage = ImageUrl(vectorImage.elementAt(k).toString());
				//}
				...
				separador.setMargin(5, -3, 0, -3);
				allContent.add(separador);
				allContent.add(info);
				add(allContent);  
				k++;
				imagen1 = null;
