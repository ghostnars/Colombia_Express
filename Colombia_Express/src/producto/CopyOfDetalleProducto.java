...
public class CopyOfDetalleProducto extends Metodos implements FieldChangeListener {
	...
	public CopyOfDetalleProducto(String titulo,String descripcion, String imagen,String precio){
		Nombre 		= titulo;
		Imagen 		= imagen;
		...
		if (Display.getWidth() == 640)
		{
			...
			AnchoImagen = 480;	
			AltoImagen = 58;
			imgWeb = "<img width='640' height='412'  src='"+Imagen+"' style='border:0px solid ORANGE;margin-left: -8px;margin-top: -8px;margin-right: -8px;margin-bottom: -8px;'>";
			izquierda = 0;
			derecha = 50;
        ...
        browserField.setMargin(0, derecha, 0, izquierda);
       // add(browserField);
        //Dialog.alert(imgWeb);
        browserField.displayContent(imgWeb, "http://localhost");
				...
				HorizontalFieldManager allContent = new HorizontalFieldManager(Field.USE_ALL_WIDTH | HorizontalFieldManager.FIELD_HCENTER);
			...
			//	pContent.setBorder(BorderFactory.createBitmapBorder(new XYEdges(25,25,25,25), bordes));
				...
				dContent.setBackground(BackgroundFactory.createBitmapBackground(imgDetalle));
			    ...
			    dContent.add(lblPregular);
				dContent.add(lblDescripcion);
				
				pContent.add(dContent);
				add(pContent);
